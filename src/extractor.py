...
class CompanyDataExtractor:
    ...
    def extract_address_with_truncation(self, pages: List[PageContent]):
        ...
        label_re = re.compile(
            ...
            r"office\s+address|company\s+address|visiting\s+address|postal\s+address|address)\s*[:\-]?\s*"
        )

        for page in pages:
            category = "legal" if any(k in page.url for k in ["imprint", "legal", "contact", "company-information"]) else "general"
            ...
            for blob in text_blobs:
                for m in label_re.finditer(blob):
                    ...
                    for raw in after.split("\n"):
                        ...
                        stopped = False
                        # Check stop patterns and truncate inline
                    ...
                    if lines:
                        ...
                        # Extract UK postcode independently if present
                        uk_pc = validate_uk_postcode(cand_addr)
                        if uk_pc:
                            # Truncate address after postcode match
                        ...
                        # Extract City / Locality candidates
                        # Common pattern: Street, Locality/City, County, Postcode
                        addr_parts = [p.strip() for p in cand_addr.split(",") if p.strip()]
                        if len(addr_parts) >= 3:
                            # Typically the city/town precedes the county or postcode
                            possible_city = addr_parts[-3] if len(addr_parts) >= 4 else addr_parts[-2]
                            # Clean possible city if it has postcode
